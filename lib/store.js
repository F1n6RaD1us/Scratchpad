...
// 早期版本的数据存在 KV（binding 名 DOCS）里：读到 D1 里没有的文档时，会自动整篇搬过来（见 migrateFromKv）。
...
export async function getDoc(env, id) {
  ...
  const doc = await db
    ...
    .bind(id)
    .first();
  return doc ?? migrateFromKv(env, db, id);
}

// 旧文档在 KV 里：第一次访问时连同历史版本一起搬进 D1。KV 里的原数据不删，留作备份
async function migrateFromKv(env, db, id) {
  if (!env.DOCS) return null;
  const doc = await env.DOCS.get(`doc:${id}`, 'json');
  if (!doc) return null;

  const history = (await env.DOCS.get(`history:${id}`, 'json')) || [];
  await db.batch([
    db.prepare('INSERT OR IGNORE INTO docs (id, content, edit_key, updated_at) VALUES (?, ?, ?, ?)')
      .bind(id, doc.content, doc.editKey, doc.updatedAt),
    ...history.map((v) => db
      .prepare('INSERT OR IGNORE INTO history (doc_id, updated_at, started_at, author, content) VALUES (?, ?, ?, ?, ?)')
      .bind(id, v.updatedAt, v.startedAt ?? v.updatedAt, v.by ?? null, v.content)),
  ]);
  return { content: doc.content, editKey: doc.editKey, updatedAt: doc.updatedAt };
}

// 返回 false 表示 ID 已被占用
export async function createDoc(env, id, content, editKey) {
  const db = await database(env);
  const { meta } = await db
    .prepare('INSERT OR IGNORE INTO docs (id, content, edit_key, updated_at) VALUES (?, ?, ?, ?)')
    .bind(id, content, editKey, Date.now())
    .run();
