...
- 每个开着的文档页：心跳每 5 秒 1 次，拉取别人保存的新内容每 30 秒 1 次，合计约**每小时 840 次**。切到后台的标签页不发
- 打开文档、保存、看历史：每次 1～2 次，和上面比可以忽略

10 万 ÷ 840 ≈ 120 个「标签页 · 小时」，比如 15 个人每天各开 8 小时就差不多到顶。超额后当天文档页和所有接口都会报错（文档页 `/doc/...` 也是 Worker 返回的），只有首页这类纯静态页面还能打开。
...
- 在线人数：每个开着的文档页约 10 秒写 1 行（服务端节流）

一个标签页开一小时大约写 360 行，比 Workers 请求次数宽松，一般不会先用完。D1 超额后保存和心跳会失败，查看文档不受影响。

### 防刷

任何人都能调用新建文档和心跳接口，有人用脚本狂刷的话，会把上面两项额度提前用完（不会丢数据，第二天自动恢复）。如果被滥用，可以在 Cloudflare 里给 `POST /api/doc` 加一条速率限制规则（域名 → 安全性 → WAF）。规则是加在域名上的，所以要先绑自定义域名，`xxx.pages.dev` 默认地址加不了。
