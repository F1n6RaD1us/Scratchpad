...
`functions/` 里的接口（Pages Functions）底层跑在 Workers 上，每次调用都算一次 Workers 请求。首页、样式、Vditor 这些静态文件不走 Worker，不计数（见上文 `_routes.json`）。
...
- 打开文档约 3～4 次，保存、看历史各 1 次，和上面比可以忽略

10 万 ÷ 240 ≈ 400 个「标签页 · 小时」，比如 50 个人每天各开 8 小时就差不多到顶。超额后当天文档页和所有接口都会报错（文档页 `/doc/...` 也是 Worker 返回的），只有首页这类纯静态页面还能打开。
...
但被拒绝的请求也算一次 Workers 请求，有人用脚本狂刷的话，照样能把请求额度提前用完（不会丢数据，第二天自动恢复）。真被滥用了，可以在 Cloudflare 里给 `/api/*` 加一条按 IP 的速率限制规则（域名 → 安全性 → WAF），正常访客每个标签页每分钟只有几次请求。规则是加在域名上的，所以要先绑自定义域名，`xxx.pages.dev` 默认地址加不了。
